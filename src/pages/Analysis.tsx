...
const Analysis = ({ onOpenEntry }: AnalysisProps) => {
    ...
    const handleOpenDetailModal = (type: 'income' | 'expense', cat: string | null = null) => {
        setDetailModalType(type);
        setDetailCategory(cat);
    };

    const handleTransactionClick = (transaction: any, type: '収入' | '支出') => {
        onOpenEntry('default', { ...transaction, transactionType: type });
                        ...
                        <div className="space-y-6">
                            {categoryData.map((data, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="space-y-2 group cursor-pointer hover:bg-[#F7F4F0]/30 -mx-4 px-4 py-2 rounded-2xl transition-colors"
                                        onClick={() => handleOpenDetailModal('expense', data.name)}
                                    >
                        ...
                        <div className="p-6 pb-4 border-b border-[#3D2B1F]/5 flex justify-between items-center bg-[#F7F4F0]/50">
                            <div>
                                <h3 className="font-black text-lg flex items-center gap-2">
                                    {detailModalType === 'income' ? '売上' : '経費'}詳細
                                    {detailCategory && <span className="text-sm bg-white px-2 py-0.5 rounded-full border border-[#3D2B1F]/10">{detailCategory}</span>}
                                    <span className="text-xs opacity-50 ml-1">({selectedYear}年)</span>
                                </h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
                            <div className="space-y-1">
                                {detailModalType === 'income' ? (
                                    ...
                                    (() => {
                                        const displayExpenses = detailCategory
                                            ? filteredExpenses.filter(e => (e.category || '未分類') === detailCategory)
                                            : filteredExpenses;

                                        return displayExpenses.length === 0 ? (
                                            <p className="text-sm text-[#3D2B1F]/40 text-center py-10 font-bold">まだ該当する経費がありません</p>
                                        ) : (
