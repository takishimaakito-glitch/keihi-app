    ...
    return (
        <div onClick={onClick} className="flex justify-between items-center p-4 bg-[#F7F4F0]/30 hover:bg-[#F7F4F0] border border-[#3D2B1F]/5 group cursor-pointer transition-colors rounded-2xl">
            <div className="flex items-center space-x-4">
                <div className={`w-2.5 h-2.5 rounded-full ${isIncome ? 'bg-emerald-500' : 'bg-[#3D2B1F]/20'}`}></div>
                <div>
                    <p className="text-sm font-bold text-[#3D2B1F] truncate max-w-[150px] sm:max-w-[200px]">{name}</p>
                    <p className="text-[10px] opacity-40 font-bold mt-1">
                        {date}  {category && `• ${category}`}
                    </p>
                </div>
            </div>
            <p className={`text-base font-black tracking-tighter ${isIncome ? 'text-emerald-700' : 'text-[#3D2B1F]'}`}>{amount}</p>
        </div>
    );
}
