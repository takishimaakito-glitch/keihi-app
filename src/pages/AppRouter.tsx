...
import { Home, List, PieChart, Settings, Plus, FileText, Camera, Pen } from 'lucide-react';
        ...
        <Router>
            <div className="min-h-screen bg-[#F7F4F0] font-sans text-[#3D2B1F] flex flex-col pb-24 relative overflow-x-hidden">
                ...
                <header className="p-6 flex justify-between items-center transition-all duration-500 max-w-3xl mx-auto w-full px-6 lg:px-12">
                    ...
                    <Link to="/settings" className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-[#3D2B1F] hover:scale-105 transition-transform z-30 relative relative relative z-20">
                        <Settings size={18} />
                    </Link>
                </header>
                ...
                {/* --- 3. ボトムナビゲーション（固定） --- */}
                <nav className="fixed bottom-0 inset-x-0 h-20 bg-white/90 backdrop-blur-md border-t border-[#3D2B1F]/5 px-8 flex justify-between items-center z-50 rounded-t-[32px] shadow-[0_-10px_40px_rgba(61,43,31,0.05)] max-w-3xl mx-auto w-full">
                    <NavItem to="/" icon={<Home size={22} />} label="ホーム" onClick={() => setIsFabMenuOpen(false)} />
                    <NavItem to="/transactions" icon={<List size={22} />} label="履歴" onClick={() => setIsFabMenuOpen(false)} />
                    ...
                    <NavItem to="/categories" icon={<PieChart size={22} />} label="カテゴリ" onClick={() => setIsFabMenuOpen(false)} />
                    <NavItem to="/invoices" icon={<FileText size={22} />} label="請求書" onClick={() => setIsFabMenuOpen(false)} />
                </nav>
...
const NavItem = ({ to, icon, label, onClick }: { to: string, icon: React.ReactNode, label: string, onClick?: () => void }) => {
    ...
    return (
        <Link
            to={to}
            onClick={onClick}
            className={`flex flex-col items-center space-y-1 transition-colors ${isActive ? 'text-[#3D2B1F] opacity-100' : 'text-[#3D2B1F] opacity-40 hover:opacity-70'}`}
        >
            {icon}
