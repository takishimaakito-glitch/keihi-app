...
const CategoryManagement = () => {
    ...
    const [newCatAllowRatio, setNewCatAllowRatio] = useState(true);
                    ...
                    <div className="w-full max-w-sm bg-white rounded-[48px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div className="space-y-4 pt-4 border-t border-[#F7F4F0]">
                                ...
                                {/* Allow Ratio Toggle */}
                                <div className="flex items-center justify-between px-2 pt-2 cursor-pointer" onClick={() => setNewCatAllowRatio(!newCatAllowRatio)}>
                                    <div className="flex flex-col">
                                        <span className="text-sm font-black text-[#3D2B1F]">事業割合を入力する</span>
                                        <span className="text-[10px] font-bold opacity-40">家賃や通信費など、プライベートと按分する項目</span>
                                    </div>
                                    <div className={`w-12 h-6 rounded-full p-1 transition-colors relative ${newCatAllowRatio ? 'bg-emerald-500' : 'bg-[#E5E0DA]'}`}>
                                        <div className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${newCatAllowRatio ? 'translate-x-6' : 'translate-x-0'}`} />
                                    </div>
                                </div>
                            </div>
