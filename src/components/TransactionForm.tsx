...
import type { Expense, Income, Favorite } from '../contexts/AppContext';
...
const TransactionForm = ({ onClose, initialMode = "default", initialData }: { onClose?: () => void; initialMode?: "default" | "camera"; initialData?: EditTransactionData | null }) => {
    const { addExpense, updateExpense, deleteExpense, addIncome, updateIncome, deleteIncome, apiKey, categories, favorites } = useAppContext();
    ...
    // New fields
    const [ratio, setRatio] = useState(initialData && initialData.transactionType === '支出' ? (initialData.ratio ?? 100) : 100);
    const [withholding, setWithholding] = useState(initialData && initialData.transactionType === '収入' ? (initialData.withholding ?? 0).toString() : '');
...
"reasoning": "判定の理由（簡潔に）"`
            ...
            const promptText = isExpense
                ? `あなたは優秀な経理アシスタントです。提供された領収書またはレシートの画像から、経費精算に必要な情報を正確に抽出してJSONのみを返してください（前置き・説明などは一切不要です）。
以下の厳密な抽出ルールに従ってください：

1. "date": 発行日・取引日を探し、必ず "YYYY-MM-DD" 形式にすること。年が省略されている場合は過去1年以内の妥当な年を推測すること。
2. "store": 店舗名・購入先名。インボイス登録番号の横やロゴ部分のテキストを優先し、支店名がある場合は「株式会社〇〇 〇〇店」のように含めること。
3. "amount": 数値のみ。"小計" や "税抜" ではなく、必ず "税込合計金額" (お買上額、領収金額、合計金額) を抽出すること。カンマは不要。
4. "memo": 購入した具体的な商品名やサービス名（例：「書籍代」「飲食代」「タクシー代」「PC周辺機器」など）。一番主要な品目を短く要約すること。

{
  ...
  "memo": "購入内容の簡潔な説明（主要品目など）",
  "is_business_expense": boolean,
  "reasoning": "経費として適切かどうかの理由"
}
${categoriesPrompt}
${expenseJudgmentPrompt}`
                : `あなたは優秀な経理アシスタントです。提供された請求書または領収書の画像から、事業収入の管理に必要な情報を正確に抽出してJSONのみを返してください（前置き・説明などは一切不要です）。
以下の厳密な抽出ルールに従ってください：

1. "date": 請求日、または支払日（発行日）を探し、必ず "YYYY-MM-DD" 形式にすること。
2. "client": 請求先・発注元の会社名・依頼主名。ロゴ部分や宛名ではなく「発行元の企業名・個人名（あなたにお金を支払う側）」を正確に抽出すること。
3. "amount": 数値のみ。"小計" ではなく、必ず "税込合計金額" (ご請求金額) を抽出すること。カンマは不要。
4. "withholding": 源泉徴収税額（復興特別所得税を含む）が記載されている場合はその金額（数値）。記載がない、または不明な場合は 0 とすること。
5. "memo": 提供した業務内容（例：「Webサイト制作費」「〇〇月分 コンサルティング報酬」など）を短く要約すること。

{
  ...
  "memo": "業務内容の簡潔な要約"
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const finalRatio = selectedCategoryData?.allowRatio ? ratio : 100;
        ...
        try {
            if (type === '支出') {
                ...
                const incomeData = {
                    ...
                    amount: parseInt(amount, 10),
                    client: clientStore,
                    withholding: withholding ? parseInt(withholding, 10) : 0,
                    memo,
                    imageUrl: previewImage || null
                };
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-24 pt-4">
                    ...
                    <div className="mb-6 rounded-2xl overflow-hidden border-2 border-[#F7F4F0] relative group bg-[#F7F4F0]/30">
                        <div className="h-40 sm:h-48 flex items-center justify-center p-2 relative">
                            ...
                            <img
                                src={previewImage}
                                alt="Receipt Preview"
                                className={`max-h-full max-w-full object-contain rounded-xl shadow-sm cursor-pointer hover:opacity-80 transition-opacity ${isScanning ? 'opacity-50 blur-sm' : ''}`}
                                onClick={() => !isScanning && setFullScreenImage(previewImage)}
                            />
                        </div>
                        <button
                ...
                <form id="tx-form" onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="bg-white border border-[#3D2B1F]/10 rounded-[28px] overflow-hidden shadow-sm divide-y divide-[#F7F4F0]">
                        ...
                        {/* Ratio (Slider - Only for allowed Expenses) */}
                        {type === '支出' && categories.find(c => c.id === category)?.allowRatio && (
                            <div className="px-5 py-4 transition-colors focus-within:bg-[#F7F4F0]/30 border-t border-[#F7F4F0]/50">
                                <div className="flex items-center justify-between mb-3">
                                    <label className="text-xs font-bold opacity-40">事業割合</label>
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="text-[10px] font-bold opacity-30 w-8 text-right">0%</span>
                                    <div className="flex-1 relative flex items-center h-6">
                                        {/* Custom Track Background */}
                                        <div className="absolute left-0 right-0 h-2 bg-[#F7F4F0] rounded-full overflow-hidden">
                                            <div
                                                className="absolute left-0 top-0 bottom-0 bg-emerald-400 rounded-full transition-all duration-150"
                                                style={{ width: `${ratio}%` }}
                                            />
                                        </div>
                                        <input
                                        ...
                                        {/* Thumb visual */}
                                        <div
                                            className="w-5 h-5 bg-white border border-[#3D2B1F]/10 rounded-full shadow-[0_2px_8px_rgba(0,0,0,0.1)] absolute pointer-events-none transition-all duration-150"
                                            style={{ left: `calc(${ratio}% - 10px)` }}
                                        />
                                    </div>
                                    <span className="text-[10px] font-bold opacity-30 w-8">100%</span>
                                </div>
                                <p className="text-[10px] font-bold opacity-40 text-center mt-2">
                                    経費になる金額: <span className="text-[#3D2B1F] opacity-100">¥{Math.round((parseInt(amount || '0') * ratio) / 100).toLocaleString()}</span>
                                </p>
                            </div>
                        )}

                        {/* Withholding Tax (Only for Incomes) */}
                        {type === '収入' && (
                            <div className="flex items-center px-5 py-2 transition-colors focus-within:bg-[#F7F4F0]/30 min-h-[60px]">
                                <label className="text-xs font-bold opacity-40 w-24 shrink-0">源泉徴収税</label>
                                <div className="flex-1 flex items-center justify-end">
                                    <span className="text-sm font-bold mr-1 opacity-50">¥</span>
                                    <input
                                        type="number"
                                        value={withholding}
