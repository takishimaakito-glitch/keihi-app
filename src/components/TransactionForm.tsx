...
import { X, Loader2, Sparkles, ArrowRight, Image as ImageIcon } from 'lucide-react';
...
const TransactionForm = ({ onClose }: { onClose?: () => void }) => {
    const { addExpense, addIncome, apiKey, categories } = useAppContext();
                ...
                : `この請求書/領収書から情報を抽出してJSONのみ返してください（前置き・説明不要）:
{
  ...
  "memo": "業務内容の簡潔な説明"
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (type === '支出') {
                const { error: submitError } = await addExpense({
                    date,
                ...
                const { error: submitError } = await addIncome({
                    date,
                    amount: parseInt(amount, 10),
                    client: clientStore,
                    withholding: 0,
    ...
    return (
        <div className="w-full flex flex-col h-[85vh] text-[#3D2B1F]">
            ...
            <div className="px-6 py-4 flex justify-between items-center bg-white shrink-0">
                <div className="bg-[#F7F4F0] p-1 rounded-full flex">
                    <button
                        onClick={() => setType('支出')}
                        className={`px-6 py-2 rounded-full text-xs font-black transition-all ${type === '支出' ? 'bg-[#3D2B1F] text-white shadow-md' : 'text-[#3D2B1F]/40'}`}
                    >
                        支出
                    </button>
                    <button
                        onClick={() => setType('収入')}
                        className={`px-6 py-2 rounded-full text-xs font-black transition-all ${type === '収入' ? 'bg-[#3D2B1F] text-white shadow-md' : 'text-[#3D2B1F]/40'}`}
                    >
                        収入
                    </button>
                </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-[#F7F4F0] p-6 pb-8 flex items-center justify-between z-10 shrink-0">
                ...
                <button
                    ...
                    className={`flex-1 ml-4 h-14 rounded-full font-black text-sm flex items-center justify-center shadow-2xl transition-all ${isSubmitting ? 'bg-[#3D2B1F]/50 text-white cursor-not-allowed' : 'bg-[#3D2B1F] text-white hover:bg-black hover:scale-[1.02] hover:-translate-y-1'}`}
                >
                    {isSubmitting ? '保存中...' : (
                        <>
                            <span>保存する</span>
